const CACHE='swe-tournament-5v5-v39-63';
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>(k.startsWith('tournoi-foot-')||k.startsWith('swe-tournament-5v5-')) && k!==CACHE).map(k=>caches.delete(k)));
